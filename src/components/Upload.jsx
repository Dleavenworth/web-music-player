...
class Upload extends Component {
	...
	createNewEntry = () => {
		if (this.state.artist !== "" && this.state.genre !== "") {
			...
			this.setState({
				...
				length: "",
				name: ""
			}, () => this.props.handleAddedSong("undefined", "/", "songList"))
		}
	}
	...
	newFile = (e) => {
		if (this.fileInput.files.item(0)) {
			...
			id3.fromFile(this.fileInput.files.item(0)).then((tags) => {
				console.log(this.unicodeToChar(tags.title).replace(/\s/g, ""))
				console.log(tags.title.toString().replace(/\s/g, ""))
				let testString = "B a d b o i"
				console.log(testString.replace(/\s/g, ""))
			})
