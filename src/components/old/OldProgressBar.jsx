...
export default class ProgressBar extends Component {
	...
	updateProgress = () => {
		if (this.state.isPlay && !this.state.seeking) {
			...
			let newProgress = Math.round(
				(newTime / this.audioRef.current.duration) * 100
			)
	...
	handleSeek = (event, newValue) => {
		if (this.state.audioSrc) {
			...
			let newTime = Math.round(
				(newValue * this.audioRef.current.duration) / 100
			)
		...
		if (prevProps.curSong !== this.props.curSong) {
			this.setState({
				audioSrc:
					...
					(this.state.usePort ? ":" + this.state.port : "") +
					"/audio/" +
					this.props.curSong.fileID,
	...
	setCurIndex = () => {
		...
		for (let i = 0; i < this.state.songList.length; i++) {
			...
			if (this.state.songList[i]._id === this.state.curSong.id) {
				...
				if (
					this.state.shuffle &&
					this.state.shuffleList[this.state.shuffleList.length - 1] !== i
				) {
			...
			if (this.state.shuffle) {
				if (this.state.curShuffleIndex - 1 >= 0) {
					...
					this.setState(
						...
						() => {
							previousSongIndex =
								this.state.shuffleList[this.state.curShuffleIndex]
							console.log(
								"previous song index inside of of: " + previousSongIndex
							)
	...
	nextShuffleOn = () => {
		...
		this.setState({ curShuffleIndex: this.state.curShuffleIndex + 1 }, () => {
			...
			if (this.state.curShuffleIndex > this.state.shuffleList.length - 1) {
				nextSongIndex = Math.round(
					Math.random() * (this.state.songList.length - 1)
				)
	...
	handleVolumeChange = (event, newValue) => {
		this.setState({ volumeSlider: newValue }, () =>
			console.log(this.state.volumeSlider)
		)
					...
					<Grid container spacing={1}>
						<Grid item xs={4}>
							<Typography variant="body1" sx={{ textAlign: "left" }}>
								{this.state.curSong !== undefined
									? this.state.curSong.name + " by " + this.state.curSong.artist
						...
						<Grid item xs={4}>
							<BottomNavigation>
								...
								<BottomNavigationAction
									label="Play"
									icon={
										this.state.isPlay && this.state.audioSrc != null ? (
											<Pause />
										) : (
											<PlayArrow />
										)
								...
								<BottomNavigationAction
									label="Loop"
									icon={!this.state.repeat ? <Repeat /> : <RepeatOn />}
